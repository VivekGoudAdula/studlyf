...
const MockInterview: React.FC = () => {
  ...
  const sendQuery = async (text: string, sessionId: string, roundIndex: number) => {
    if (text) {
      setChatHistory(prev => [...prev, { role: 'candidate', content: text }]);
    }
    setLoading(true);
    try {
      const endpoint = session?.rounds[roundIndex].round_type === 'hr_voice'
      ...
      // Handle Voice Output if HR round
      if (session?.rounds[roundIndex].round_type === 'hr_voice' && interviewerText) {
        speak(interviewerText);
      }
    } catch (error) {
  ...
  const startVoiceCapture = () => {
    ...
    recognitionRef.current.onresult = (event: any) => {
      ...
      if (session) sendQuery(transcript, session._id, session.current_round_index);
    };
  ...
  const nextRound = () => {
    ...
    if (nextIndex < session.rounds.length) {
      ...
      sendQuery('', session._id, nextIndex);
    } else {
      generateReport();
    }
  };
  ...
  const LandingView = () => (
    <div className="max-w-6xl mx-auto pt-16">
      ...
      >
        ...
        <span className="text-xl font-bold uppercase tracking-[0.3em] relative z-10">Initialize Setup Protocol</span>
      ...
      <div className="max-w-2xl mx-auto pt-16">
        ...
        >
          ...
          <div className="space-y-8">
            ...
            <button
              onClick={() => startInterview(formData)}
            ...
            >
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <span>Start Simulation</span>}
            </button>
          </div>
        </motion.div>
      </div>
          ...
          <div className="relative group">
            {isVoice ? (
              <div className="p-8 bg-indigo-900 rounded-[3rem] shadow-2xl flex flex-col items-center">
                ...
                <h4 className="text-white font-bold mb-2 uppercase tracking-widest text-xs">Voice Interaction Mode</h4>
                <p className="text-indigo-300 text-[10px] mb-8 uppercase tracking-widest font-bold">Speak naturally to response</p>
              ...
              <div className="flex items-center space-x-3">
                <input
                  ...
                  className="flex-grow p-6 bg-white rounded-3xl border border-gray-100 shadow-lg focus:ring-2 focus:ring-indigo-500 outline-none font-medium"
                  value={inputText}
                  onChange={e => setInputText(e.target.value)}
                  onKeyPress={e => e.key === 'Enter' && sendQuery(inputText, session._id, session.current_round_index)}
                />
                <button
                  onClick={() => sendQuery(inputText, session._id, session.current_round_index)}
                  disabled={!inputText || loading}
                  className="p-6 bg-gray-900 text-white rounded-3xl shadow-xl hover:bg-black transition-all disabled:opacity-50"
                >
